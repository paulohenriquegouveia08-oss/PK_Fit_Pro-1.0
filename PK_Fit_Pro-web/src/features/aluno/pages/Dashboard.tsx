...
import { getLastPresence } from '../../../shared/services/turnstile.service';
...
export default function Dashboard() {
    ...
    const [lastPresence, setLastPresence] = useState<{ created_at: string; access_granted: boolean } | null>(null);
    ...
    useEffect(() => {
        const loadData = async () => {
            ...
            if (studentId) {
                ...
                if (academyId) {
                    const presenceResult = await getLastPresence(studentId, academyId);
                    if (presenceResult.success && presenceResult.data) {
                        setLastPresence(presenceResult.data);
                    }
                }
            }
            setIsLoading(false);
        ...
        <DashboardLayout title="Dashboard" menuItems={menuItems}>
            <div className="aluno-dashboard">
                ...
                <div className="stats-grid">
                    ...
                    <div className="stat-card">
                        <div className={`stat-icon ${lastPresence ? 'primary' : 'warning'}`}>
                            <svg width="24" height="24" viewBox="0 0 24 24" fill="currentColor">
                        ...
                        <div className="stat-content">
                            <div className="stat-value">
                                {lastPresence 
                                    ? new Date(lastPresence.created_at).toLocaleDateString('pt-BR', { day: '2-digit', month: '2-digit' }) 
                                    : '-'}
                            </div>
                            <div className="stat-label">Última Presença</div>
                        </div>
                    </div>
                </div>
