...
import { validateInviteCode, type ValidateInviteResult } from '../../shared/services/invite.service';
...
export default function LoginPage() {
    ...
    const getStepTitle = () => {
        switch (step) {
            ...
            case 'invite-input':
                return 'Insira seu código de convite';
            case 'invite-form':
                return 'Complete seu Cadastro';
        }
    };

    const handleInviteValidation = async (e: FormEvent) => {
        e.preventDefault();
        setError('');
        setIsLoading(true);
        ...
        try {
            ...
            if (!result.success || !result.data?.valid) {
                setError(result.data?.reason || 'Código de convite inválido ou expirado');
                return;
            }
    ...
    return (
        <div className="login-container">
            <div className="login-card">
                    ...
                    <form className="login-form" onSubmit={handleEmailSubmit}>
                        ...
                        <div style={{ marginTop: 'var(--spacing-4)', textAlign: 'center' }}>
                            <button 
                                ...
                                onClick={() => { setError(''); setStep('invite-input'); }}
                            >
                ...
                {step === 'invite-input' && (
                    <form className="login-form" onSubmit={handleInviteValidation}>
                        <button type="button" className="back-button" onClick={handleBack}>
                            <svg width="16" height="16" viewBox="0 0 24 24" fill="currentColor">
                                <path d="M10.828 12l4.95 4.95-1.414 1.414L8 12l6.364-6.364 1.414 1.414z" />
                            </svg>
                        ...
                        <div className="form-group">
                            <label className="form-label" htmlFor="inviteCode">Código de Convite</label>
                            <input
                                id="inviteCode"
                                type="text"
                                className={`form-input ${error ? 'error' : ''}`}
                        ...
                        <button
                            type="submit"
                            className={`btn btn-primary btn-full ${isLoading ? 'btn-loading' : ''}`}
                            disabled={isLoading || inviteCode.length < 12}
                        >
                            {isLoading ? '' : 'Validar Convite'}
                        </button>
                    </form>
                )}

                {step === 'invite-form' && inviteData && (
                    <InviteForm 
                        inviteCode={inviteCode} 
                        inviteData={inviteData} 
                        onSuccess={handleInviteSuccess} 
                        onBack={() => setStep('invite-input')} 
                    />
                )}
            </div>
        </div>
