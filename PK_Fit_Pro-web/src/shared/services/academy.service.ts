...
export async function createAcademy(data: CreateAcademyData): Promise<ApiResponse<Academy>> {
    if (USE_CORE_API) {
        const res = await apiRequest<{ academy_id: string; admin_id: string }>('/api/v1/academies', {
            method: 'POST',
            body: JSON.stringify({
                name: data.name,
                admin_name: data.responsible_name,
        ...
        return getAcademyById(res.data!.academy_id);
    }
...
export async function deleteAcademy(id: string): Promise<ApiResponse<void>> {
    if (USE_CORE_API) {
        return apiRequest<void>(`/api/v1/academies/${id}`, { method: 'DELETE' });
    }
