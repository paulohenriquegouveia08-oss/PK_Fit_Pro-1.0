...
export async function createAcademyMember(data: CreateMemberData): Promise<ApiResponse<User>> {
    if (USE_CORE_API) {
        try {
            const res = await apiRequest<{ id: string; email: string; role: string }>('/api/v1/users', {
                method: 'POST',
                body: JSON.stringify({
                    name: data.name,
                    email: data.email,
            ...
            if (data.photo_url) {
                await supabase.from('users').update({ photo_url: data.photo_url }).eq('id', res.data!.id);
            }
            
            const { data: user } = await supabase.from('users').select('*').eq('id', res.data!.id).single();
