...
export async function createPayment(data: CreatePaymentData): Promise<ApiResponse<Payment>> {
    if (USE_CORE_API) {
        return apiRequest<Payment>('/api/v1/billing/payments', {
            method: 'POST',
...
export async function markStudentPlanAsPaid(
    ...
    if (USE_CORE_API) {
        return apiRequest<Payment>('/api/v1/billing/payments/paid', {
            method: 'POST',
            body: JSON.stringify({
...
export async function markStudentPlanAsUnpaid(
    ...
    if (USE_CORE_API) {
        const res = await apiRequest<any>('/api/v1/billing/payments/unpaid', {
            method: 'POST',
            body: JSON.stringify({
                student_id: studentId,
                plan_id: planId
