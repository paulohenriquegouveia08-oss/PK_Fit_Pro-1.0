...
// Validar acesso do aluno via RPC (mais rápido possível) ou API
export async function validateStudentAccess(
    academyId: string,
    userId: string
): Promise<ApiResponse<AccessValidationResult>> {
    if (USE_CORE_API) {
        return apiRequest<AccessValidationResult>('/api/v1/access/validate', {
            method: 'POST',
...
export async function registerAccessLog(log: {
    ...
    if (USE_CORE_API) {
        return apiRequest<AccessLog>('/api/v1/access/logs', {
            method: 'POST',
...
export async function getLastPresence(userId: string, academyId: string): Promise<ApiResponse<{ created_at: string; access_granted: boolean } | null>> {
    if (USE_CORE_API) {
        return apiRequest<{ created_at: string; access_granted: boolean } | null>('/api/v1/access/last-presence');
    }
    ...
    try {
        ...
        console.error('Error getting last presence:', error);
        return { success: false, error: 'Erro ao buscar última presença' };
    }
}
