...
export class HenryAdapter implements TurnstileAdapter {
    ...
    private async httpRequest(endpoint: string, method: string = 'GET', body?: unknown): Promise<unknown> {
    ...
    async connect(): Promise<void> {
        ...
        try {
            ...
            logger.warn(`[Henry] ⚠️ TCP não disponível: ${tcpError instanceof Error ? tcpError.message : tcpError}`);
    ...
    private scheduleTcpReconnect(): void {
        ...
        logger.info(`[Henry] Reconectando TCP em ${this.reconnectDelay / 1000}s... (${this.reconnectAttempts}/${this.maxReconnectAttempts})`);
