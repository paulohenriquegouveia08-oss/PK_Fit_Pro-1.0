...
export class TopDataAdapter implements TurnstileAdapter {
  ...
  private scheduleReconnect(): void {
    if (this.reconnectAttempts >= this.maxReconnectAttempts) {
      logger.error(
        `[Top Data] ❌ Máximo de tentativas de reconexão atingido (${this.maxReconnectAttempts})`
      )
      return
    }
    ...
    logger.info(
      `[Top Data] Reconectando em ${this.reconnectDelay / 1000}s... (tentativa ${this.reconnectAttempts}/${this.maxReconnectAttempts})`
    )
    ...
    this.reconnectTimer = setTimeout(async () => {
      try {
        ...
        logger.error(
          `[Top Data] Falha na reconexão: ${error instanceof Error ? error.message : error}`
        )
      }
    }, this.reconnectDelay)
  }
  ...
  private sendCommand(command: Buffer): void {
    ...
    try {
      ...
      logger.error(
        `[Top Data] Erro ao enviar comando: ${error instanceof Error ? error.message : error}`
      )
    }
  }
  ...
  private processPacket(packet: Buffer): void {
    ...
    logger.debug(
      `[Top Data] Pacote recebido: cmd=0x${commandByte?.toString(16)} [${packet.toString('hex')}]`
    )
  ...
  async grantAccess(direction: 'IN' | 'OUT'): Promise<void> {
    const command =
      direction === 'IN' ? TD_COMMANDS.GRANT_CLOCKWISE : TD_COMMANDS.GRANT_ANTICLOCKWISE
  ...
  async syncUserFace(userId: string, name: string, _photoUrl: string): Promise<void> {
    ...
    try {
      ...
      const packet = Buffer.from([
        ...
        0x0a,
        0x08,
        ...Buffer.from(String(numericId).padStart(10, '0')),
        0x03
      ])
