...
class Logger {
    ...
    public onLogCallback?: (logEntry: { level: string, message: string, data?: unknown, timestamp: string }) => void;
    ...
    access(granted: boolean, userName: string, reason: string): void {
        if (this.onLogCallback) {
            this.onLogCallback({ level: granted ? 'success' : 'error', message: `${granted ? '✅ ACESSO LIBERADO' : '❌ ACESSO NEGADO'} ${userName} — ${reason}`, timestamp: this.formatTimestamp() });
        }
        if (granted) {
